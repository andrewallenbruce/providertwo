...
method(base_request, current) <- function(x, query = NULL, years = NULL) {
  identifier_(x) |>
    request() |>
...
method(base_request, temporal) <- function(x, query = NULL, years = NULL) {
  if (is_empty(years)) {
    return(
      identifier_(x) |>
        map(
...
method(query_nresults2, current) <- function(x) {
...
method(query_nresults2, temporal) <- function(x) {
